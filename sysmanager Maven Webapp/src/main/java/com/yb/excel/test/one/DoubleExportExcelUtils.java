...
public class DoubleExportExcelUtils {  
	...
	public void exportExcel(HSSFWorkbook workbook, int sheetNum,  
        ...
        if (result != null) {  
            ...
            for (List<Object> m : result) {  
                ...
                int cellIndex = 0;  
                for (Object str : m) {  
                	 HSSFCell textcell = row.createCell((short) cellIndex);
                	if(str!=null){
						if(str instanceof Integer){
							textcell.setCellValue((Integer)(str));
						}else if (str instanceof Double) {
							textcell.setCellValue((Double)(str));
						}else{ // 如果type是类类型，则前面包含"class "，后面跟类名  
					        textcell.setCellValue(str.toString());
					    }
					}else {
