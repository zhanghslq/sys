...
public class TagGroupController {
	...
	public List<Group> queryAll(){
		...
		for (TagGroup tagGroup : queryAll) {
			...
			Integer total = vipTagService.queryTotal(loyalty, identity, gender, age, type, coupon, recentOil, recentNotOil, shortOil, station, oilName, shopName, mopType,list3);
	...
	public void exportById(Integer id,HttpServletResponse response){
		...
		if(tagGroup!=null){
	        ...
	        while (true) {//死循环
	           num++;
	           list = vipTagService.query(loyalty, identity, gender, age, type, coupon, recentOil, recentNotOil, shortOil, station, oilName, shopName, mopType,start, 60000,list3);
